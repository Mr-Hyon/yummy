...
public class ConfirmOrderJob {
    ...
    public void execute() throws JobExecutionException {
        System.out.println("配送订单检测任务开始执行!");
        Queue<MyOrder> queue = new LinkedList<>();

        // 在每次启动Job时去数据库查找失效订单,并加入到队列中(从数据库中查询，此处使用假数据)
        List<MyOrder> list = orderService.getDeliveredOrder();
        ...
        while (element != null) {
            ...
            if (diff != null && diff >= EFFTIVE_TIME) {
                System.out.println("开始结束已完成订单" + element.getId() + "配送时间" + element.getDeliverTime());
                orderService.confirmOrder(element);
                // 弹出队列
                queue.poll();
                // 取下一个元素
                element = queue.peek();
                ...
                try {
                    System.out.println("等待检测订单" + element.getId() + "配送时间" + element.getDeliverTime() + "已配送"
                            + diff / 1000 + "秒");
                    //线程等待
                    Thread.sleep(EFFTIVE_TIME - diff);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                    logger.info("ConfirmOrderJob.checkOrder定时任务出现问题");
                }
            }
        }
    }
    ...
    public Long checkOrder(MyOrder myOrder) {
        ...
        if (myOrder != null) {
            try {
                Date deliverTime = sdf.parse(myOrder.getDeliverTime());
                diff = sdf.parse(sdf.format(date)).getTime() - sdf.parse(sdf.format(deliverTime)).getTime();
